...
public class User implements Serializable {
     ...
     * List of Nhinrep
     */
    private List<Nhinrep> nhinreps = null;
    /**
     * List of Userrole
     */
     ...
     * - Conformance Code -
     */
    private LabConstants.LabType labType;
    ...
    public Participant getParticipant() {
        ...
        if (this.participants != null && this.participants.size() > 0) {
            participant = this.participants.get(0);
        }
        return participant;
    }
    ...
    public Nhinrep getNhinrep() {
        ...
        if (this.nhinreps != null && this.nhinreps.size() > 0) {
            nhinrep = this.nhinreps.get(0);
        }
        return nhinrep;
    }

    /**
     * Get the list of Nhinrep
     */
    public List<Nhinrep> getNhinreps() {
        return this.nhinreps;
    }

    /**
     * Set the list of Nhinrep
     */
    public void setNhinreps(List<Nhinrep> nhinreps) {
        this.nhinreps = nhinreps;
    }
    ...
    public boolean isAdministrator() {
        int primaryRole = getPrimaryRole();
        return (primaryRole== 1 || primaryRole==2);
    }

    public String getUserSpecificName() {
        String specificName = this.username;

        if (this.participants != null && this.participants.size() > 0) {
            specificName = this.participants.get(0).getParticipantName();
        }
        if (this.nhinreps != null && this.nhinreps.size() > 0) {
            specificName = this.nhinreps.get(0).getName();
        }
