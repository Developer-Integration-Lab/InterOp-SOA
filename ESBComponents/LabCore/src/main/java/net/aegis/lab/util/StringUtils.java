...
public final class StringUtils
{
	...
	public static final DecimalFormat PST_DEFAULT_DECIMAL_FORMAT = new DecimalFormat("###,###,##0.00");
	public static final NumberFormat PST_DEFAULT_NUMBER_FORMAT =  NumberFormat.getIntegerInstance();

    //ENCODING

    /**
     * Returns a string that is the CSV encoding of the argument string,
     * namely, the argument string enclosed in double-quotes, with any
     * embedded double quotes escaped by an additional preceding double quote.
     *
    ...
    public static String csvEncode(String string)
    {
        ...
        {
            if (string.charAt(i) == '"') stringBuffer.append('"');
    ...
    /**
     * Returns a string representation of the argument object,
     * enclosed in double quotes.
     *
     * @param object the object to be represented as a double-quoted string
     *
     * @return a string consisting of a double-quoted string representation of the argument
     */
    ...
    /**
     * Returns a string that is the HTML encoding of the argument string,
    ...
    public static String indentByWith(String aString, int aNumberOfLevels, String aSequence)
    {
    ...
    public static void indentByWith(StringBuffer aBuffer, int aNumberOfLevels, String aSequence)
    {
    ...
    /**
     * Returns a new String of the indicated length
     * whose characters are all spaces.
    ...
    public static String leftPadIn(String aValue, int stringSize, char padChar)
    {
        if (isNullOrEmpty(aValue) || stringSize == 0)
            return aValue;
        return org.apache.commons.lang.StringUtils.leftPad(aValue, stringSize, padChar);
    }

    public static String rightPadIn(String aValue, int stringSize, char padChar)
    {
        if (isNullOrEmpty(aValue) || stringSize == 0)
            return aValue;
        return org.apache.commons.lang.StringUtils.rightPad(aValue, stringSize, padChar);
    }
    ...
    /**
     * Compares two String instances, allowing null in either or both strings. If both
     * strings are null, they are considered equal.  If only one string is null, the non-null
     * string is considered greater.  If both strings are non-null, this method returns the 
     * results of string1.compareTo(string2).
     * 
     * @param string1 a String instance to compare. May be null.
     * @param string2 a String instance to compare. May be null.
     * @return <0 if string1 < string2, 0 if string1.equals(string2), >0 if string1 > string2.
     */
     ...
     * @param string1 a String instance to compare. May be null.
     * @param string2 a String instance to compare. May be null.
     * @return true if the String instances are lexically equal. Otherwise false.
    ...
    /**
     * Checks two string instances for equality without case sensitivity, allowing for null in
     * either or both strings.  If both strings are null, they are considered equal.
     * 
     * @param string1 a String instance to compare. May be null.
     * @param string2 a String instance to compare. May be null.
     * @return true if the String instances are lexically equal. Otherwise false.
    ...
    /**
     * Appends the String argument to the StringBuffer,
     * followed by the delimiter, if the String argument
     * is not null or empty.
     *
     * @param element the String to append
     * @param delimiter the delimiter String to append
     * following the element String
     * @param buffer the buffer to append to
     */
    public static boolean appendElement(String element, String delimiter, StringBuffer buffer)
    {
    ...
    public static boolean isNumeric(String aString) {
        if (isNullOrEmpty(aString)) return false;
    ...
    public static boolean isAlphaNumeric(String aString) {
        ...
        CharacterIterator i = new StringCharacterIterator(aString);
        for (char c = i.first(); c != CharacterIterator.DONE; c = i.next()) {
            if (!Character.isLetter(c) && !Character.isDigit(c) && c != SPACE) return false;
        }
        return true;
    }
    ...
    public static boolean isNumericOrDecimal(String aString) 
    {
        ...
        for (char c = i.first(); c != CharacterIterator.DONE; c = i.next()) 
        {
            ...
            {
            	dotOccurrence += 1;
            	if (dotOccurrence == 2) return false;
            }
            else if (!Character.isDigit(c) && c != PERIOD) return false;
        }
        return true;
    }
    
    /**
     * Returns true only if aString is not null and is of zero length after trimming.
     * 
     * @param aString a String to check for emptiness
    ...
    /**
     * Converts a camel case string into a lower-case, delimiter-separated string. 
     * For example, a call of toSeparatedString("ACamelCaseString, '_') returns 
     * a_camel_case_string.
     * 
     * @param camelCaseString a String in camel case to delimit.
     * @param delimiter the character used to separate the string.
     * @return a String where capitals in the original are prefixed with the given delimiter.
     */
...
//            }
    ...
    /**
     * Converts the string (presumably a single word) from plural into singular form.
     * 
    ...
    /**
     * Truncates the string (presumably a large amount of text) down to the first
     * whole word after the length parameter and then adds an ellipsis (...) if 
     * there is additional text.
     * 
     * @param longString a block of text
     * @param length the desired length of the resulting text string
     * @return
     */
    public static String truncateWithEllipsis(String longString, int length)
    {
        ...
        {
            //check length of text and cut off at first whole word after specified nbr of characters (Jim's bytes)
            if (longString.length() > length)
            {
                ...
                while (checkingForWholeWord)
                {
                    if ((longString.length() > characterPosition) && 
                        (notNullOrEmpty(String.valueOf(longString.charAt(characterPosition)))))
                    {
                        characterPosition++;
                    }
                    else
                ...
                longString = longString.substring(0, characterPosition) + continuationIndicator;
            }
        }
     ...
     */
    public static String listToDelimitedString(List<String> aList, String delimiter)
    {
    ...
    /**
     * Converts argument's fractional part (after decimal) into two digits format.
     * Example: If strValue is 9.9 then return value will be 9.90 
    ...
    public static String currencyFormat(String strValue)
	{
		...
		{	
        	...
        	{
        		String digitsAfterDecimal = strValue.substring(decimalIndex+1);
     ...
     * @param strValue a decimal value to be validated
     * @param precision an integer value 
     * @param scale an integer value
     * @return boolean if strValue is within the given precision & scale values.
     */
    public static boolean validatePrecisionScale(String strValue, int precision, int scale)
    {
    	...
    	{	
	    	...
	    	else if (indexOfDecimal > -1) 
	    	{
	    		...
	    		String fractionalDigits = strValue.substring(indexOfDecimal+1, strValue.length());
    ...
    public static String stringArrayToStringWithDelimiter(String[] a, String separator) {
    ...
    // STRING VALIDATIONS
    /**
     * Method to check if the input phoneNumber is in Phone Number Format: Only 10 digits entered.
     * @param phoneNumber
     * @return boolean
     */
    ...
    public static boolean is4Digit24HourTimeFormat(String inputTime)
    {
    	if (StringUtils.isNumeric(inputTime) && inputTime.length() == 4)
    	{
			...
			if(hours >= HOURS_MIN_LIMIT && hours <= HOURS_MAX_LIMIT && 
			   minutes >= MINUTES_MIN_LIMIT	&& minutes <= MINUTES_MAX_LIMIT)
			{
				return true;
			}
    	}
    ...
    public static boolean isDurationFormat(String duration)
    {
    	...
    	if (StringUtils.notNullOrEmpty(duration))
        {
            ...
            for (char c = ci.first(); c != CharacterIterator.DONE; c = ci.next(), ++i) 
            {
                if ((i == 0 || i == 1 || i == 4 || i == 5 || i == 8 || i == 9) &&  
                	!Character.isDigit(c) && Character.isDigit(validCharArray[i]))
                {
                	return false;
                }
                else if ((i == 2 || i == 6 || i == 10) && c != validCharArray[i])
                {
                	return false;
                }
                else if ((i == 3 || i == 7) && c != COLON  && validCharArray[i] == COLON)
                {
                	return false;
                }
            }
    ...
    /**
     * Converts duration components into string array.
     * Example: 10D:05H:30M will be converted into String [] without D, H & M characters
     * @param String duration
     * @return String []
     */ 
    ...
    public static List<String> getListFromStringWithSeperator(String value, char separator){
        ArrayList<String> list = new ArrayList<String>();
        StringTokenizer tokens = new StringTokenizer(value, String.valueOf(separator));
    ...
    public static String[] getStringArrayFromStringWithSeperator(String value, char separator){
        StringTokenizer tokens = new StringTokenizer(value, String.valueOf(separator));
    ...
    /**
     * Returns a value along with trailing blank spaces according to the maxCharLength  
     * specified & ensuring that the string value has specified length. 
     * 
     * @param string value where trailing spaces needs to be appended
     * @param int value that specifies the actual length of the value   
     * @return a string in specified length
     */
    public static String getValueWithTrailingSpaces(String value, int maxCharLength)
    {
    	...
    	{
    		trailingSpaces = indentByWith(trailingSpaces, (maxCharLength - result.length()), " ");
    ...
    static public String localizedFormatForDouble(String pattern, double value, Locale loc)
    {
    ...
    {
       return PST_DEFAULT_NUMBER_FORMAT.format((aNumber == null) ? Integer.valueOf(0) : aNumber);
    }
    
    /**
     * Utility method to format a number into localized string with max fraction value (default 2).
     *   
     * @param Number aNumber
     * @return a String of formatted number
     */
    public static final String formatDecimalNumber(final Number aNumber)
    {
       ...
       return decimalFormat.format((aNumber == null) ? Integer.valueOf(0) : aNumber);
    }
    
    /**
     * Utility method to format a number into localized string with max fraction value (default 2).
     *   
     * @param Number aCurrencyValue
     * @return a String of formatted number
     */
    public static final String formatCurrency(final Number aCurrencyValue)
    {
       return NumberFormat.getCurrencyInstance().format((aCurrencyValue == null) ? Integer.valueOf(0) : aCurrencyValue);
    }
}
